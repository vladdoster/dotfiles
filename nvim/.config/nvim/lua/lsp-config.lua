    ...
    function(client, bufnr)
        ...
        if client.resolved_capabilities
            ...
            buf_set_keymap('n',
                           ...
                           opts)
        elseif client.resolved_capabilities
            .document_range_formatting then
            buf_set_keymap('n',
                           '<leader>lf',
...
elseif vim.fn.has('unix') == 1 then
    ...
    print(
        'Unsupported system for sumneko')
end
...
do
    ...
    local default_handler = vim.lsp
                                .handlers[method]
    vim.lsp.handlers[method] = function(
        err, method, result, client_id,
        bufnr, config)
        default_handler(err, method,
                        result,
                        client_id,
                        bufnr, config)
        local diagnostics = vim.lsp
                                .diagnostic
                                .get_all()
        local qflist = {}
        for bufnr, diagnostic in pairs(
                                     diagnostics) do
            for _, d in ipairs(
                            diagnostic) do
                d.bufnr = bufnr
...
--         lspconf[lang].setup {on_attach = on_attach, root_dir = vim.loop.cwd}
--     end
-- end
