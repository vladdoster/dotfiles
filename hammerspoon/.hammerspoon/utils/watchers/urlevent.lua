...
module.start = function()
    ...
    hs.urlevent.httpCallback = function(
        ...
        local shouldFocusBrowser =
            not modifiers['cmd']

        local runningBrowser =
            hs.fnutils.find(
      ...
      tell application "{APP_NAME}"
        ...
        open location "{URL}"
                ...
                URL = fullURL,
                ACTIVATE = shouldFocusBrowser
                    and 'activate' or '' -- 'activate' brings to front if cmd is clicked
            }))

        -- focus back the current app
        if not shouldFocusBrowser
            and not currentApp:isFrontmost() then
            currentApp:activate()
        end
    end
end
