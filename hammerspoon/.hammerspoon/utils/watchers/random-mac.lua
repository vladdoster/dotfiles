...
local log = hs.logger.new('random-mac',
                          'debug');
...
local randomizeMAC =
    function(_, _, _, prevNetwork,
             network)
        if network == nil and network
            ~= prevNetwork then
            log.d(
                'wi-fi connected, randomizing MAC')

            hs.execute(
                '/usr/bin/sudo /usr/local/bin/spoof randomize wi-fi',
                true)
        end
    end
