...
local events = {
    ...
    windowUnminimized=true,
    windowFullscreened=true,
    windowUnfullscreened=true, -- FIXME better names
...
function Window:emitEvent(event)
    ...
    for ww in pairs(self.wws) do
        if watchers[ww] then -- skip if wwatcher was stopped
            ...
            if fns then
                if not logged then
                    log.df('Emitting %s %d (%s)', event, self.id, self.app.name)
                    logged = true
                end
                for fn in pairs(fns) do fn(self.window, self.app.name) end
            end
        end
    end
end
...
function Window.new(win, id, app, watcher)
    ...
    app.windows[id] = o
    for ww, active in pairs(watchers) do if active then o:setWWatcher(ww) end end
...
function App:getWindows()
    ...
    for _, win in ipairs(windows) do appWindowEvent(win, hsuiwatcher.windowCreated, nil, self.name) end
    self:getFocused()
...
function App:focusChanged(id, win)
    ...
    if self.focused and self.focused.id == id then
        return log.df('Window %d (%s) already focused, skipping', id, self.name)
    end
    local active = global.active
    if not self.windows[id] then appWindowEvent(win, hsuiwatcher.windowCreated, nil, self.name) end
    ...
    if event == hsuiwatcher.windowCreated then
        ...
        if not id then
            ...
            if retry > MAX_RETRIES then
                log.df('%s: %s has no id', appname,
                       win.subrole and win:subrole() or (win.role and win:role()) or 'window')
            end
            if retry <= MAX_RETRIES then
                windowWatcherDelayed[win] = delayed.doAfter(windowWatcherDelayed[win], retry * RETRY_DELAY,
                                                            appWindowEvent, win, event, _, appname, retry)
            end
            return
        end
        ...
        if not watcher._element.pid then
            ...
            if retry > MAX_RETRIES then
                log.df('%s: %s has no watcher pid', appname,
                       win.subrole and win:subrole() or (win.role and win:role()) or 'window')
            end
            if retry <= MAX_RETRIES then
                windowWatcherDelayed[win] = delayed.doAfter(windowWatcherDelayed[win], retry * RETRY_DELAY,
        ...
        delayed.cancel(windowWatcherDelayed[win])
        windowWatcherDelayed[win] = nil
        Window.new(win, id, apps[appname], watcher)
        watcher:start({
            hsuiwatcher.elementDestroyed,
            hsuiwatcher.windowMoved,
            hsuiwatcher.windowResized,
            hsuiwatcher.windowMinimized,
            hsuiwatcher.windowUnminimized,
            hsuiwatcher.titleChanged
        })
...
local function startAppWatcher(app, appname, retry, takeiteasy)
    if not app or not appname then
        log.e('Called startAppWatcher with no app')
        return
    end
    if apps[appname] then return not takeiteasy and log.df('App %s already registered', appname) end
    if app:kind() < 0 or not isGuiApp(appname) then
        log.df('App %s has no GUI', appname)
        return
    end
    local watcher = app:newWatcher(appWindowEvent, appname)
    if watcher._element.pid then
        ...
        retry = (retry or 0) + 1
        if retry > 5 then
            return not takeiteasy and log.wf('STILL no accessibility pid for app %s, giving up', (appname or '[???]'))
        end
        log.df('No accessibility pid for app %s', (appname or '[???]'))
        appWatcherDelayed[appname] = delayed.doAfter(appWatcherDelayed[appname], 0.2 * retry, startAppWatcher, app,
                                                     appname, retry)
    end
end
...
local function appEvent(appname, event, app, retry)
    ...
    if event == hsappwatcher.launched then
        return startAppWatcher(app, appname)
    elseif event == hsappwatcher.launching then
        return
    end
    ...
    if event == hsappwatcher.terminated then
        return appo:destroyed()
    elseif event == hsappwatcher.deactivated then
        return appo:deactivated()
    elseif event == hsappwatcher.hidden then
        return appo:hidden()
    elseif event == hsappwatcher.unhidden then
        return appo:shown()
    end
end
...
local function stopGlobalWatcher()
    ...
    for _, app in pairs(apps) do
        for _, window in pairs(app.windows) do window.watcher:stop() end
        app.watcher:stop()
...
function windowwatcher:getWindows()
    ...
    for appname, app in pairs(apps) do
        for _, window in pairs(app.windows) do
            for ww in pairs(window.wws) do if ww == self then t[#t + 1] = window.window end end
        end
    end
    -- sort by id
...
function windowwatcher:subscribe(event, fn)
    ...
    elseif type(event) == 'table' then
        for _, e in ipairs(event) do subscribe(self, e, fn) end
        return self
    else
...
function windowwatcher:unsubscribe(fn)
    ...
    for _, e in ipairs(fn) do
        if type(e) == 'string' then
            unsubscribeEvent(self, e)
        elseif type(e) == 'function' then
            unsubscribe(self, e)
        else
            error('fn must be a function, string, or a table of functions or strings', 2)
        end
    end
...
function windowwatcher:resume()
    if watchers[self] == true then
        log.i('instance already running, ignoring')
        return
    end
    start(self)
end
...
function windowwatcher.switchedToSpace(space, cb)
    ...
    delayed.doAfter(0.5, function()
        ...
        for _, app in pairs(apps) do app:getWindows() end
