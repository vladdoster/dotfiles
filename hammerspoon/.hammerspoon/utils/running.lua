local log = hs.logger.new('running', 'debug')
...
module._updateAppWindows = function(app, ax)
  ...
  if ax.AXChildren then
    for _, child in ipairs(ax.AXChildren) do if child:matchesCriteria('AXWindow') then module._addAppWindow(app, child) end end
  end
...
module._watchApp = ---@param app hs.application
function(app)
  if module.apps[app:pid()] ~= nil then return end
  local ax = hs.axuielement.applicationElement(app)
  if ax:isValid() then
    log.d('## watching: ' .. app:name())
    ...
    local addWatcher = function(notif) w:addWatcher(ax, notif) end
...
module._updateSpaces = function()
  for _, space in pairs(spaces.layout()[spaces.mainScreenUUID()]) do
    for _, w in pairs(spaces.allWindowsForSpace(space)) do module._addAppWindow(w:application(), hs.axuielement.windowElement(w)) end
  end
end
module._updateRunning = function() for _, app in ipairs(hs.application.runningApplications()) do module._watchApp(app) end end
module.switcher = function()
  if module._chooser == nil then
    module._chooser = hs.chooser.new(function(choice) if choice ~= nil then choice.win:focus() end end):bgDark(true):placeholderText(
                          'Switch to Window'):searchSubText(true)
  elseif module._chooser:isVisible() then
    module._chooser:hide()
    return
  end
  local windows = hs.fnutils.map(module.getWindows(), function(win)
    local ret = {text = win:title(), subText = win:application():title(), win = win}
    if win:application():bundleID() then ret.image = hs.image.imageFromAppBundle(win:application():bundleID()) end
    return ret
  end)
...
module._appWatcher = appw.new(function(appName, event, app)
  ...
  if event == appw.hidden then module.triggerChange(app, app:mainWindow(), module.events.hidden) end
end)
