...
local notifyBattery = function(_, _, _, _, status)
    ...
    if status.isCharged ~= cache.batteryCharged and status.percentage == 100 and status.powerSource ==
      'AC Power' then
        hs.notify.new(
          {title = 'Power Status', subTitle = 'Battery: Charged', contentImage = IMAGE_PATH}
        ):send()
    ...
    if (isBurnRateHigh and status.burnRate ~= cache.burnRate) or
      (not isBurnRateHigh and cache.isBurnRateHigh) then
        local subTitle = isBurnRateHigh and 'High 🚨' or 'Back To Normal'

        hs.notify.new({title = 'Burn Rate', subTitle = subTitle, contentImage = IMAGE_PATH}):send()
