...
module._tap = hs.eventtap.new({hs.eventtap.event.types.keyDown}, function(event)
  ...
  for s = 1, 9 do if hasSpaceMods and event:getKeyCode() == hs.keycodes.map[string.format('%d', s)] then module.set(s) end end
  if hasMods and isUp then if module.active ~= 1 then module.previous() end end
  if hasMods and isDown then if module.active ~= #module.layout() then module.next() end end
end)
module._tap:start()
module._update(true)
