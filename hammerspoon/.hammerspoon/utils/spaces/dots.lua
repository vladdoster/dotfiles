...
module.draw = function()
    ...
    local screenUUIDs = {}
    hs.fnutils.each(hs.screen.allScreens(), function(screen) screenUUIDs[screen:spacesUUID()] = screen end)
    ...
    hs.fnutils.each(allUUIDs, function(screenUUID)
        ...
        for i = 1, math.max(#screenSpaces, #cache.dots[screenUUID]) do
            ...
            if not dot then
                dot = hs.drawing.circle({x=0, y=0, w=spaces.dots.size, h=spaces.dots.size}):setStroke(false)
                          :setBehaviorByLabels({'moveToActiveSpace', 'stationary'})
                          :setLevel(hs.drawing.windowLevels.desktop):setFillColor({
                        red=1.0,
                        green=1.0,
                        blue=1.0,
                        alpha=1.0
            ...
            if i <= #screenSpaces then
                local x = screenFrame.w / 2 - (#screenSpaces / 2) * spaces.dots.distance + i * spaces.dots.distance -
                              spaces.dots.size * 3 / 2
                local y = screenFrame.h - spaces.dots.distance
                local alpha = hs.fnutils.contains(activeSpaces, screenSpaces[i]) and spaces.dots.selectedAlpha or
                                  spaces.dots.alpha
                dot:setTopLeft({x=x + screenFrame.x, y=y + screenFrame.y}):setAlpha(alpha):show()
            else
...
module.stop = function()
    ...
    hs.fnutils.each(cache.dots, function(screen) hs.fnutils.each(screen, function(dot) dot:delete() end) end)
    cache.dots = {}
end
return module
