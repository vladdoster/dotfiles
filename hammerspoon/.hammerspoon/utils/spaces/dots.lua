...
local query = require(
                  'hs._asm.undocumented.spaces').query
local masks = require(
                  'hs._asm.undocumented.spaces').masks
local layout = require(
                   'hs._asm.undocumented.spaces').layout
...
module.draw = function()
    ...
    local activeSpaces = query(
                             masks.currentSpaces,
                             true)
    local cacheUUIDs = keys(cache.dots)
    local screenUUIDs = {}

    hs.fnutils.each(hs.screen
                        .allScreens(),
                    function(screen)
        screenUUIDs[screen:spacesUUID()] =
            screen
    end)
...
module.start = function()
    ...
    cache.watchers.spaces = hs.spaces
                                .watcher
                                .new(
                                module.draw):start()
    cache.watchers.screen = hs.screen
                                .watcher
                                .new(
                                module.draw):start()
...
module.stop = function()
    ...
    hs.fnutils.each(cache.dots,
                    function(screen)
        hs.fnutils.each(screen,
                        function(dot)
            dot:delete()
        end)
    end)
