...
local waitForSpaceAnimation = function(targetSpace, changedFocus, mousePosition)
    ...
    cache.waiting = hs.timer.waitUntil(
                      function()
          return (spaces.activeSpace() == targetSpace) or
                   (hs.timer.secondsSinceEpoch() - cache.changeStart > 2)
      end, function()
...
module.start = function()
    cache.eventtap = hs.eventtap.new(
                       {hs.eventtap.event.types.keyDown}, function(event)
