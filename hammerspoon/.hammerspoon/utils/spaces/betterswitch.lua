...
local activeSpaceIndex = require(
                             'ext.spaces').activeSpaceIndex
local focusScreen =
    require('ext.screen').focusScreen
local keys = require('ext.table').keys
local screenSpaces = require(
                         'ext.spaces').screenSpaces
local spaceFromIndex = require(
                           'ext.spaces').spaceFromIndex
local spaceInDirection = require(
                             'ext.spaces').spaceInDirection
...
local waitForSpaceAnimation = function(
    targetSpace, changedFocus,
    mousePosition)
    if cache.waiting then return end

    cache.changeStart = hs.timer
                            .secondsSinceEpoch()
...
module.switchToIndex = function(
    ...
    local activeIdx =
        activeSpaceIndex(
            currentScreenSpaces)
    local targetSpace =
        spaceFromIndex(targetIdx)

    -- check if we really can send the keystrokes
    local shouldSendEvents =
        hs.fnutils.every({
            not cache.switching,
            targetSpace,
            activeIdx ~= targetIdx,
    ...
    if shouldSendEvents then
        ...
        local eventCount = math.abs(
                               targetIdx
                                   - activeIdx)
        local eventDirection = targetIdx
                                   > activeIdx
                                   and 'right'
                                   or 'left'
        ...
        waitForSpaceAnimation(
            targetSpace, changedFocus,
            mousePosition)
    end
end

module.switchInDirection = function(
    direction)
    local currentScreen = activeScreen()
    local mousePosition = hs.mouse
                              .getAbsolutePosition()
    local targetSpace =
        spaceInDirection(direction)

    -- gain focus on the screen
    local changedFocus =
        focusScreen(currentScreen)
    waitForSpaceAnimation(targetSpace,
                          changedFocus,
                          mousePosition)
end
