...
local buildMenu = function(lines)
    ...
    for idx = 1, #lines - 1, 2 do
        local timeSpan = hs.styledtext.new(
                           getTimeSpan(lines[idx + 1]) .. '\t',
                           {font = {name = hs.styledtext.defaultFonts.menu, size = 10.0}}
                         )

        local taskName = hs.styledtext.new(
                           getTitle(lines[idx]),
                           {font = {name = hs.styledtext.defaultFonts.menu, size = 13.0}}
                         )
...
local updateMenubar = function()
    hs.task.new(
      '/usr/local/bin/icalBuddy', function(_, stdOut)
          ...
          if currentTimeMinutes >= startTimeMinutes then
              ...
              cache.menuItem:setTitle(
                hs.styledtext.new(
                  text, {
                      font = {name = hs.styledtext.defaultFonts.menuBar.name, size = 11.0},
                      baselineOffset = -1.0
                  }
                )
              )
      ...
      end, {'-ea', '-ic', 'Tracking', 'eventsFrom:now', 'to:today'}
    ):start()
end
