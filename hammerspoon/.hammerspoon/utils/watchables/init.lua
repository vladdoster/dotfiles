local status = hs.watchable
                   .new('status')
local log = hs.logger.new('watchables',
                          'debug')
...
local updateBattery =
    function()
        ...
        status.battery = {
            isCharged = hs.battery
                .isCharged(),
            percentage = hs.battery
                .percentage(),
            powerSource = hs.battery
                .powerSource(),
            amperage = hs.battery
                .amperage(),
            burnRate = burnRate
...
local updateScreen =
    function()
        ...
        status.isThunderboltConnected =
            hs.screen.findByName(
                'LG QHD') ~= nil
        status.isLaptopScreenConnected =
            hs.screen.findByName(
                'Color LCD') ~= nil

        log.d('updated screens:',
              hs.inspect(
                  status.connectedScreenIds))
    end
...
local updateUSB = function()
    status.isErgodoxAttached =
        hs.fnutils.find(hs.usb
                            .attachedDevices(),
                        function(device)
            log.d('updated ergodox:',
                  status.isErgodoxAttached)
...
module.start = function()
    ...
    cache.watchers = {
        ...
        screen = hs.screen.watcher.new(
            updateScreen):start(),
        sleep = hs.caffeinate.watcher
            .new(updateSleep):start(),
        usb = hs.usb.watcher.new(
            updateUSB):start(),
        -- network = cache.configuration:monitorKeys({ VPN_CONFIG_KEY, NETWORK_SHARING }):setCallback(updateNetwork):start(),
        wifi = hs.wifi.watcher.new(
            updateWiFi):start()
    }
...
module.stop = function()
    hs.fnutils.each(cache.watchers,
                    function(watcher)
        watcher:stop()
    end)
