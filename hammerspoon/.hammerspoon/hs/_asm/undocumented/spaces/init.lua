    ...
    function(obj, key)
        if _kMetaTable._k[obj] then
            if _kMetaTable._k[obj][key] then
                return
                    _kMetaTable._k[obj][key]
            else
                for k, v in pairs(
                                _kMetaTable._k[obj]) do
                    if v == key then
                        return k
                    end
                end
...
_kMetaTable.__newindex =
    function(obj, key, value)
        error(
            'attempt to modify a table of constants',
            2)
        return nil
    end
_kMetaTable.__pairs =
    function(obj)
        return
            pairs(_kMetaTable._k[obj])
    end
_kMetaTable.__tostring =
    function(obj)
        local result = ''
        if _kMetaTable._k[obj] then
            local width = 0
            ...
            result =
                'constants table missing'
        end
        return result
    end
_kMetaTable.__metatable = _kMetaTable -- go ahead and look, but don't unset this

local _makeConstantsTable = function(
    theTable)
...
local reverseWithoutSystemSpaces =
    function(list)
        local results = {}
        for i, v in ipairs(list) do
            if internal.spaceType(v)
                ~= internal.types.system then
...
local isSpaceSafe = function(spaceID,
                             func)
    func = func or 'undocumented.spaces'
    if not _BE_DANGEROUS_FLAG_ then
        local t = internal.spaceType(
                      spaceID)
        if t ~= internal.types
            .fullscreen and t
            ~= internal.types.tiled and t
            ~= internal.types.user then
            _BE_DANGEROUS_FLAG_ = false
            error(func
                      .. ':must be user-created or fullscreen application space',
                  3)
        end
    end
    _BE_DANGEROUS_FLAG_ = false
...
local screenMT = hs.getObjectMetatable(
                     'hs.screen')
local windowMT = hs.getObjectMetatable(
                     'hs.window')

-- Public interface ------------------------------------------------------

module.types = _makeConstantsTable(
                   internal.types)
module.masks = _makeConstantsTable(
                   internal.masks)
...
module.count = function()
    return #reverseWithoutSystemSpaces(
               module.query(
                   internal.masks
                       .allSpaces))
end
...
module.moveToSpace =
    function(whichIndex)
        local theID =
            internal.query(
                internal.masks.allSpaces)[whichIndex]
        if theID then
            internal._changeToSpace(
                theID, false)
            return true
        else
            return false
    ...
    function(...)
        ...
        if args.n == 0 then
            ...
            for i, v in ipairs(
                ...
                isAnimating =
                    isAnimating
                        or internal.screenUUIDisAnimating(
                            internal.UUIDforScreen(
                                v))
            end
            return isAnimating
        elseif args.n == 1 then
            return
                internal.screenUUIDisAnimating(
                    internal.UUIDforScreen(
                        args[1]))
        else
            error(
                'isAnimating:invalid argument, none or hs.screen object expected',
                2)
        end
    end

module.spacesByScreenUUID =
    function(...)
        local args = table.pack(...)
        if args.n == 0 or args.n == 1 then
            ...
            for i, v in
                ipairs(theSpaces) do
                local myScreen =
                    internal.spaceScreenUUID(
                        v)
                        or 'screenUndefined'
                if not holding[myScreen] then
                    holding[myScreen] =
                        {}
                end
                table.insert(
            ...
            error(
                'spacesByScreenUUID:invalid argument, none or integer expected',
                2)
        end
    end

-- need to make sure its a user accessible space
module.changeToSpace =
    function(...)
        local args = table.pack(...)
        if args.n == 1 or args.n == 2 then
            ...
            if fromID == 0 then
                error(
                    'changeToSpace:unable to identify screen for space id '
                        .. spaceID, 2)
            end
            ...
            error(
                'changeToSpace:invalid argument, spaceID and optional boolean expected',
                2)
        end
...
module.createSpace =
    function(...)
        ...
        if args.n <= 2 then
            ...
            error(
                'createSpace:invalid argument, screenUUID and optional boolean expected',
                2)
        end
    end
...
module.removeSpace =
    function(...)
        ...
        if args.n == 1 or args.n == 2 then
            ...
            if internal.spaceType(
                spaceID)
                ~= internal.types.user then
                error(
                    'removeSpace:you can only remove user created spaces',
                    2)
            end
            for i, v in ipairs(
                            module.query(
                ...
                if spaceID == v then
                    error(
                        'removeSpace:you can\'t remove one of the currently active spaces',
                        2)
                end
            end
            local targetUUID =
                internal.spaceScreenUUID(
                    spaceID)
            local sameScreenSpaces =
                module.spacesByScreenUUID()[targetUUID]
            ...
            if userSpacesCount < 2 then
                error(
                    'removeSpace:there must be at least one user space on each screen',
                    2)
            end
            ...
            for i, v in
                ipairs(theWindows) do
                -- only add windows that exist in only one place
                if #internal.windowsOnSpaces(
                    v) == 1 then
                    internal.windowsAddTo(
                        v, baseID)
                end
            end

            internal.windowsRemoveFrom(
                theWindows, spaceID)
            internal._removeSpace(
                spaceID)
            if resetDock then
                hs.execute(
                    'killall Dock')
            ...
            error(
                'removeSpace:invalid argument, spaceID and optional boolean expected',
                2)
        end
    end
...
module.allWindowsForSpace =
    function(...)
        local args = table.pack(...)
        if args.n == 1 then
            local ok, spaceID = pcall(
                                    isSpaceSafe,
                                    args[1],
                                    'allWindowsForSpace')
            if not ok then
                if internal.spaceName(
                    args[1])
                    == 'dashboard' then
                    spaceID = args[1]
                else
            ...
            error(
                'allWindowsForSpace:invalid argument, spaceID expected',
                2)
        end
    end

module.windowOnSpaces =
    function(...)
        local args = table.pack(...)
        if args.n == 1 then
            windowIDs =
            ...
            error(
                'windowOnSpaces:invalid argument, windowID expected',
                2)
        end
    end

module.moveWindowToSpace =
    function(...)
        local args = table.pack(...)
        if args.n == 2 then
            ...
            if #currentSpaces == 0 then
                error(
                    'moveWindowToSpace:no spaceID found for window',
                    2)
            elseif #currentSpaces > 1 then
                error(
                    'moveWindowToSpace:window on multiple spaces',
                    2)
            end
            if currentSpaces[1]
                ~= spaceID then
            ...
            error(
                'moveWindowToSpace:invalid argument, windowID and spaceID expected',
                2)
        end
    end
...
module.query = function(...)
    ...
    if args.n <= 2 then
        ...
        error(
            'query:invalid argument, mask and optional boolean expected',
            2)
    end
end
...
module.screensHaveSeparateSpaces =
    internal.screensHaveSeparateSpaces
module.activeSpace =
    internal.activeSpace
module.spaceType = internal.spaceType
module.spaceName = internal.spaceName
module.spaceOwners =
    internal.spaceOwners
module.spaceScreenUUID =
    internal.spaceScreenUUID
...
module.debug.report =
    function(...)
        ...
        elseif table.pack(...).n ~= 0 then
            error(
                'debugReport:bad mask type provided, expected number',
                2)
        end
...
module.debug.spaceInfo =
    function(v)
        local results =
            'Space: ' .. v .. ' ('
                .. inspect(
                    internal.spaceName(v))
                .. ')\n'
                .. '    Type:      '
                .. (module.types[internal.spaceType(
                    v)]
                    and module.types[internal.spaceType(
                        v)]
                    or '-- unknown --')
                .. ' ('
                ...
                .. ')\n'
                .. '    Level:     '
                .. internal.spaceLevel(v)
                .. '\n'
                .. '    CompatID:  '
                .. internal.spaceCompatID(
                    v) .. '\n'
                .. '    Screen:    '
                .. inspect(
                    internal.spaceScreenUUID(
                ...
                .. '    MShape:    '
                .. (inspect(
                    internal.spaceManagedShape(
                        v)):gsub('%s+',
                                 ' '))
                .. '\n'
                .. '    Transform: '
                .. (inspect(
                    internal.spaceTransform(
                        v)):gsub('%s+',
                                 ' '))
                .. '\n'
                .. '    Values:    '
                .. (inspect(
                    internal.spaceValues(
                        v)):gsub('%s+',
                                 ' '))
                .. '\n'
                .. '    Owners:    '
            ...
            for i, v in ipairs(
                ...
                table.insert(apps,
                             (application.applicationForPID(
                                 v)
                                 and application.applicationForPID(
                                     v):title()
                                 or 'n/a'))
            end
            results = results
...
screenMT.__index.spaces =
    function(obj)
        return
            module.spacesByScreenUUID()[internal.UUIDforScreen(
                obj)]
    end
screenMT.__index.spacesUUID =
    internal.UUIDforScreen
screenMT.__index.spacesAnimating =
    function(obj)
        return
            internal.screenUUIDisAnimating(
                internal.UUIDforScreen(
...
windowMT.__index.spaces =
    function(obj)
        return obj:id()
                   and internal.windowsOnSpaces(
                       obj:id()) or nil
    end
windowMT.__index.spacesMoveTo =
    function(obj, ...)
        if obj:id() then
            module.moveWindowToSpace(
                obj:id(), ...)
...
if settings.get(
    '_ASMundocumentedSpacesRaw') then
    module.raw = internal
    module.raw.changeToSpace =
        function(...)
