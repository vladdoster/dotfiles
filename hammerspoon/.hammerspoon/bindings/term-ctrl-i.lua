...
local rebindCtrlI = function(appName, options)
    ...
    if cache.bindings[appName] then
        ...
        cache.bindings[appName] = hs.hotkey.bind({'ctrl'}, 'i', function() hs.eventtap.keyStroke({}, 'F6') end)
    end
end
module.start = function()
    cache.filter = hs.window.filter.new({'Terminal', 'kitty'})
    cache.filter:subscribe({hs.window.filter.windowFocused, hs.window.filter.windowUnfocused},
                           function(_, appName, event) rebindCtrlI(appName, {
        enabled=(event == 'windowFocused')
    }) end)
end
