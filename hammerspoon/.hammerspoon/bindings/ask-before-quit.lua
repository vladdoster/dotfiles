local allWindowsCount = require(
                            'ext.application').allWindowsCount

local cache = { bindings = {} }
local module = { cache = cache }

-- ask before quitting app when there are multiple windows
local askBeforeQuitting = function(
    appName, options)
    local enabled = options.enabled
                        or false

    if not enabled
        and cache.bindings[appName] then
        ...
        cache.bindings[appName] =
            hs.hotkey.bind({ 'cmd' },
                ...
                if windowsCount > 1 then
                    ...
                    hs.task.new(
                        os.getenv('HOME')
                            .. '/.hammerspoon/assets/ask-before-quit.scpt',
                        nil, { appName }):start()
                else
                    hs.application.find(
                        appName):kill()
