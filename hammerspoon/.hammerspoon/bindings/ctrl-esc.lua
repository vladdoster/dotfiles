...
module.start = function()
    ...
    cache.controlKeyTimer = hs.timer.delayed.new(
                              CANCEL_DELAY_SECONDS, function()
          cache.sendEscape = false
      end
                            )
    ...
    cache.controlTap = hs.eventtap.new(
    ...
    cache.keyDownEventTap = hs.eventtap.new(
                              {hs.eventtap.event.types.keyDown}, function()
          cache.sendEscape = false
          return false
      end
                            )
    ...
    cache.watcher = hs.watchable.watch(
                      'status.isErgodoxAttached', function(_, _, _, _, isErgodoxAttached)
