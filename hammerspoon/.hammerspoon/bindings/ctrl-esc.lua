...
module.start = function()
    ...
    cache.controlKeyTimer = hs.timer.delayed.new(CANCEL_DELAY_SECONDS, function() cache.sendEscape = false end)
    ...
    cache.controlTap = hs.eventtap.new({hs.eventtap.event.types.flagsChanged}, function(event)
        ...
        -- any action
        if cache.lastModifiers['ctrl'] == newModifiers['ctrl'] then return false end
