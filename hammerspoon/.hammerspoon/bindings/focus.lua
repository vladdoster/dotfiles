...
local focusAndHighlight = function(cmd)
    ...
    elseif screenInDirection then  -- focus screen in given direction if exists
        focusScreen(screenInDirection)
    elseif #filterWindows > 0 then -- focus first window if there are any
        forceFocus(filterWindows[1])
    else                           -- finally focus the screen if nothing else works
        focusScreen(focusedScreen)
    end
end
module.start = function()
    local bind = function(key, fn) hs.hotkey.bind({'ctrl', 'alt'}, key, fn, nil, fn) end
    cache.focusFilter = hs.window.filter.new():setCurrentSpace(true):setDefaultFilter():keepActive()
    hs.fnutils.each({
        {key = 'h', cmd = 'west'},  {key = 'j', cmd = 'south'},
        {key = 'k', cmd = 'north'}, {key = 'l', cmd = 'east'}
    }, function(object) bind(object.key, function() focusAndHighlight(object.cmd) end) end)
