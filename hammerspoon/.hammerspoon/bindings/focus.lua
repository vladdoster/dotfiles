...
local cycleWindows = require(
                         'ext.window').cycleWindows
local focusScreen =
...
local focusAndHighlight =
    function(cmd)
        local focusedWindow = hs.window
                                  .focusedWindow()
        local focusedScreen =
            activeScreen()
        ...
        local windowsToFocus =
            cache.focusFilter[winCmd](
                cache.focusFilter,
                focusedWindow,
                ONLY_FRONTMOST,
                STRICT_ANGLE)
        local screenInDirection =
            focusedScreen[screenCmd](
                focusedScreen)
        local filterWindows =
            cache.focusFilter:getWindows()

        local windowOnSameOrNextScreen =
            function(testWin,
                     currentScreen,
                     nextScreen)
                return
        ...
        if #windowsToFocus > 0
            and windowOnSameOrNextScreen(
                windowsToFocus[1],
                focusedScreen,
                screenInDirection) then
...
module.start = function()
    ...
    cache.focusFilter =
        hs.window.filter.new():setCurrentSpace(
            true):setDefaultFilter()
            :keepActive()
