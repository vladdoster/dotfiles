...
local smartLaunchOrFocus = require(
                               'ext.application').smartLaunchOrFocus
...
module.start = function()
    ...
    hs.fnutils.each({
        {
            key = '/',
            fn = system.toggleConsole
        }, {
            key = 'b',
            fn = system.toggleBluetooth
        },
        {
            key = 'd',
            fn = system.toggleDND
        ...
        {
            key = 't',
            fn = system.toggleTheme
        },
        {
            key = 'w',
            fn = system.toggleWiFi
        }
    }, function(object)
        hs.hotkey.bind(ultra,
                       object.key,
                       object.fn)
    end)
    ...
    hs.fnutils.each({
        {
            key = 'return',
            apps = config.apps.terms
        }, {
            key = 'space',
            apps = config.apps.browsers
        }, {
            key = ',',
            apps = {
                'System Preferences'
