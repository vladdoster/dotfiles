...
return function(hhtwm)
    ...
    layouts['floating'] = function() return nil end
    ...
    layouts['main-left'] = function(window, windows, screen, index, layoutOptions)
        if #windows == 1 then return layouts['main-center'](window, windows, screen, index, layoutOptions) end
        local margin = hhtwm.margin or 0
        local insetFrame = getInsetFrame(screen)
        local frame = {x=insetFrame.x, y=insetFrame.y, w=0, h=0}
        if index == 1 then
    ...
    layouts['main-center'] = function(window, windows, screen, index, layoutOptions)
        ...
        local frame = {x=insetFrame.x, y=0, w=(insetFrame.w - mainColumnWidth) / 2 - margin, h=0}
        if (index - 1) % 2 == 0 then
