...
local log = hs.logger.new('hhtwm',
                          'debug');

local SWAP_BETWEEN_SCREENS = false

local getDefaultLayoutOptions =
    function()
        return { mainPaneRatio = 0.5 }
    end
...
local ternary = function(cond, ifTrue,
                         ifFalse)
    if cond then
        return ifTrue
    else
...
local ensureCacheSpaces = function(
    spaceId)
    if spaceId
        and not cache.spaces[spaceId] then
        cache.spaces[spaceId] = {}
    end
end

local getCurrentSpacesIds =
    function()
        return spaces.query(
                   spaces.masks
                       .currentSpaces)
...
local getSpaceId = function(win)
    ...
    if win ~= nil and win:spaces() ~= nil
        and #win:spaces() > 0 then
        spaceId = win:spaces()[1]
    end

    return spaceId
               or spaces.activeSpace()
end

local getSpacesIdsTable =
    function()
        local spacesLayout =
            spaces.layout()
        local spacesIds = {}
...
local getAllWindowsUsingSpaces =
    function()
        local spacesIds =
            getSpacesIdsTable()
        ...
        hs.fnutils.each(spacesIds,
                        function(spaceId)
            local windows =
                spaces.allWindowsForSpace(
                    spaceId)

            hs.fnutils.each(windows,
                            function(win)
...
local getScreenBySpaceId = function(
    spaceId)
    ...
    return hs.fnutils.find(
            ...
            return hs.fnutils.contains(
                       spacesLayout[spaceUUID],
                       spaceId)
        end)
end

local getCurrentSpacesByScreen =
    function()
        local currentSpaces =
            spaces.query(
                spaces.masks
...
module.findTrackedWindow =
    function(win)
        ...
        for spaceId, spaceWindows in
            pairs(cache.spaces) do
            for winIndex, window in
                pairs(spaceWindows) do
...
module.getLayout = function(spaceId)
    ...
    for screenUUID, layoutSpaces in
        pairs(layout) do
        if not foundScreenUUID then
            if hs.fnutils.contains(
                layoutSpaces, spaceId) then
                foundScreenUUID =
                    screenUUID
            end
        end
    ...
    local screen = hs.fnutils.find(
                       hs.screen
                           .allScreens(),
                       function(screen)
            return screen:spacesUUID()
                       == foundScreenUUID
        end)

    return cache.layouts[spaceId]
               or (screen
                   and module.displayLayouts
                   and module.displayLayouts[screen:id()])
               or (screen
                   and module.displayLayouts
                   and module.displayLayouts[screen:name()])
               or 'monocle'
end
...
module.resizeLayout =
    function(resizeOpt)
        ...
        local calcResizeStep =
            module.calcResizeStep
                or function()
                    return 0.1
                end
        local screen =
            getScreenBySpaceId(spaceId)
        local step =
            calcResizeStep(screen)
        local ratio =
            cache.layoutOptions[spaceId]
                .mainPaneRatio
        ...
        elseif resizeOpt == 'thinner' then
            ratio =
                math.max(ratio - step, 0)
        elseif resizeOpt == 'wider' then
            ratio =
                math.min(ratio + step, 1)
        end
...
module.swapInDirection =
    function(win, direction)
        win = win
                  or hs.window
                      .frontmostWindow()
        ...
        windowsInDirection =
            hs.fnutils.filter(
                ...
                function(testWin)
                    return
                        testWin:isStandard()
                            and not module.isFloating(
                                testWin)
                end)

        if #windowsInDirection >= 1 then
            local winInDirection =
                windowsInDirection[1]
            ...
            local winInDirectionScreen =
                winInDirection:screen()
            local winScreen =
                win:screen()
            ...
            if not SWAP_BETWEEN_SCREENS
                and winScreen:id()
                ~= winInDirectionScreen:id() then
                return
            end
            ...
            if winScreen:id()
                ~= winInDirectionScreen:id() then
                win:moveToScreen(
                    winInDirectionScreen)
                winInDirection:moveToScreen(
                    winScreen)
            end
            ...
            cache.spaces[winSpaceId][winIdx] =
                winInDirection
            cache.spaces[winInDirectionSpaceId][winInDirectionIdx] =
                win
...
module.throwToScreen =
    function(win, direction)
        ...
        if not directions[direction] then
            log.e(
                'can\'t throw in direction:',
                direction)
            return
        end

        local screen = win:screen()
        local screenInDirection =
        ...
        if screenInDirection then
            ...
            if cache.spaces[winSpaceId] then
                table.remove(
                    cache.spaces[winSpaceId],
                    winIdx)
            else
                log.e(
                    'throwToScreen no cache.spaces for space id:',
                    winSpaceId)
            end
...
module.throwToScreenUsingSpaces =
    function(win, direction)
        win = win
                  or hs.window
                      .frontmostWindow()
        ...
        local screenInDirection =
            screen[directions[direction]](
                screen)
        local currentSpaces =
            getCurrentSpacesByScreen()
        local throwToSpaceId =
            currentSpaces[screenInDirection:id()]
        ...
        if hs.fnutils.some({
            ...
            log.e(
                'throwToScreenUsingSpaces error',
                hs.inspect(
                    { winSpaceId, winIdx }));
            return
        end
        ...
        if cache.spaces[winSpaceId] then
            ...
            log.e(
                'throwToScreenUsingSpaces no cache.spaces for space id:',
                winSpaceId)
        end

        local newX =
            screenInDirection:frame().x
        local newY =
            screenInDirection:frame().y

        spaces.moveWindowToSpace(
            win:id(), throwToSpaceId)
...
module.throwToSpace =
    function(win, spaceIdx)
        if not win then
            log.e(
                'throwToSpace tried to throw nil window')
            return false
        end
        ...
        if not spaceId then
            log.e(
                'throwToSpace tried to move to non-existing space',
                spaceId,
                hs.inspect(spacesIds))
            return false
        end
        ...
        if module.isFloating(win) then
            ...
            local newX =
                win:frame().x
                    - win:screen()
                        :frame().x
                    + targetScreen:frame().x
            local newY =
                win:frame().y
                    - win:screen()
                        :frame().y
                    + targetScreen:frame().y
        ...
        if cache.spaces[winSpaceId] then
            ...
            log.e(
                'throwToSpace no cache.spaces for space id:',
                winSpaceId)
        end
...
module.isFloating = function(win)
    local trackedWin, _, _ =
        module.findTrackedWindow(win)
    local isTrackedAsTiling =
        trackedWin ~= nil
    ...
    local isTrackedAsFloating =
        hs.fnutils.find(cache.floating,
                        function(
            floatingWin)
            return floatingWin:id()
                       == win:id()
        end)

    -- if window is not floating and not tiling, then default to floating?
    if isTrackedAsFloating == nil
        and trackedWin == nil then
        return true
    end
...
module.toggleFloat =
    function(win)
        ...
        if module.isFloating(win) then
            ...
            for index, floatingWin in
                pairs(cache.floating) do
            ...
            table.insert(
                cache.spaces[spaceId],
                win)
            table.remove(cache.floating,
                         foundIdx)
        else
            local foundWin, winSpaceId,
                  winIdx =
                module.findTrackedWindow(
                    win)
            ...
            if cache.spaces[winSpaceId] then
                ...
                log.e(
                    'window made floating without previous :space()',
                    hs.inspect(foundWin))
            end

            table.insert(cache.floating,
                         win)
        end
...
module.tile = function()
    ...
    hs.fnutils.each(allWindows or {},
        ...
        if shouldFloat(win) then
            ...
            table.insert(tilingWindows,
                         win)
        end
    end)

    -- add new tiling windows to cache
    hs.fnutils.each(tilingWindows,
                    function(win)
        ...
        if not trackedWin
            or trackedSpaceId ~= spaceId then
    ...
    hs.fnutils.each(currentSpaces,
        ...
        for i = #spaceWindows, 1, -1 do
            ...
            if not existsOnScreen
                or duplicateIdx > 0 then
                table.remove(
                    spaceWindows, i)
            else
                i = i + 1
    ...
    hs.fnutils.each(floatingWindows,
                    function(win)
    ...
    hs.fnutils.each(currentSpaces,
        ...
        hs.fnutils.each(
            ...
            function(screen)
                ...
                if not layoutName
                    or not layouts[layoutName] then
                    log.e(
    ...
    hs.fnutils.each(moveToFloat,
                    function(win)
        local _, spaceId, winIdx =
            module.findTrackedWindow(win)
...
module.detectTile = function(win)
    ...
    local shouldTileDefault =
        hs.axuielement.windowElement(win):isAttributeSettable(
            'AXSize')
...
local loadSettings =
    function()
        -- load from cache
        local jsonTilingCache =
            hs.settings.get(
                'hhtwm.tilingCache')
        local jsonFloatingCache =
            hs.settings.get(
                'hhtwm.floatingCache')

        log.d('reading from hs.settings')
        log.d('hhtwm.tilingCache',
              jsonTilingCache)
        log.d('hhtwm.floatingCache',
              jsonFloatingCache)

        -- all windows from window filter
        local allWindows =
            getAllWindowsUsingSpaces()

        local findWindowById = function(
            winId)
            return hs.fnutils.find(
                       allWindows,
                       function(win)
        ...
        if jsonTilingCache then
            local tilingCache = hs.json
                                    .decode(
                                    jsonTilingCache)
            local spacesIds =
                getSpacesIdsTable()

            hs.fnutils.each(tilingCache,
                            function(obj)
                ...
                if hs.fnutils.contains(
                    ...
                    cache.spaces[obj.spaceId] =
                        {}
                    cache.layouts[obj.spaceId] =
                        obj.layout
                    cache.layoutOptions[obj.spaceId] =
                        obj.layoutOptions
                    ...
                    hs.fnutils.each(
                        ...
                        function(winId)
                            ...
                            log.d(
                                'restoring (spaceId, windowId, window)',
                                obj.spaceId,
                                winId,
                                win)
                            if win then
        ...
        log.d('cache.spaces',
              hs.inspect(cache.spaces))
        log.d('cache.floating',
              hs.inspect(cache.floating))
    end
...
local saveSettings =
    function()
        ...
        for spaceId, spaceWindows in
            ...
            if #spaceWindows > 0 then
                ...
                for _, window in pairs(
                                     spaceWindows) do
                    log.d(
                        'storing (spaceId, windowId, window)',
                        spaceId,
                        window:id(),
                        window)
                    table.insert(tmp,
        ...
        local jsonTilingCache = hs.json
                                    .encode(
                                    tilingCache)
        local jsonFloatingCache =
            hs.json.encode(floatingCache)

        log.d('storing to hs.settings')
        log.d('hhtwm.tiling',
              jsonTilingCache)
        log.d('hhtwm.floating',
              jsonFloatingCache)

        hs.settings
            .set('hhtwm.tilingCache',
                 jsonTilingCache)
        hs.settings.set(
            'hhtwm.floatingCache',
            jsonFloatingCache)
...
module.start = function()
    ...
    -- start window filter
    cache.filter = hs.window.filter
                       .new():setDefaultFilter()
                       :setOverrideFilter(
                           {
