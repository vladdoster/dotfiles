...
local getSpacesIdsTable = function()
    ...
    hs.fnutils.each(
      hs.screen.allScreens(), function(screen)
          local spaceUUID = screen:spacesUUID()

          local userSpaces = hs.fnutils.filter(
                               spacesLayout[spaceUUID], function(spaceId)
...
local getCurrentSpacesByScreen = function()
    ...
    hs.fnutils.each(
      hs.screen.allScreens(), function(screen)
          ...
          local visibleSpace = hs.fnutils.find(
                                 screenSpaces, function(spaceId)
...
module.setLayout = function(layout, spaceId)
    ...
    cache.floating = hs.fnutils.filter(
                       cache.floating, function(win)
...
module.getLayout = function(spaceId)
    ...
    return cache.layouts[spaceId] or
             (screen and module.displayLayouts and module.displayLayouts[screen:id()]) or
             (screen and module.displayLayouts and module.displayLayouts[screen:name()]) or
             'monocle'
end
...
module.swapInDirection = function(win, direction)
    ...
    windowsInDirection = hs.fnutils.filter(
                           windowsInDirection, function(testWin)
    ...
    if #windowsInDirection >= 1 then
        ...
        if hs.fnutils.some(
          {win, winInDirection, winInDirectionSpaceId, winInDirectionIdx, winSpaceId, winIdx},
          function(_)
              return _ == nil
          end
        ) then
            log.e(
              ...
              hs.inspect({winInDirectionSpaceId, winInDirectionIdx, winSpaceId, winIdx})
            );
            return
        end
...
module.isFloating = function(win)
    ...
    local isTrackedAsFloating = hs.fnutils.find(
                                  cache.floating, function(floatingWin)
...
module.tile = function()
    ...
    hs.fnutils.each(
      currentSpaces, function(spaceId)
          ...
          for i = #spaceWindows, 1, -1 do
              ...
              local existsOnScreen = hs.fnutils.find(
                                       tilingWindows, function(win)
              ...
              if duplicateIdx > 0 then
                  log.e(
                    'duplicate idx',
                    hs.inspect({i = i, duplicateIdx = duplicateIdx, spaceWindows = spaceWindows})
                  )
              end
    ...
    cache.floating = hs.fnutils.filter(
                       cache.floating, function(cacheWin)
          return hs.fnutils.find(
                   floatingWindows, function(win)
    ...
    hs.fnutils.each(
      currentSpaces, function(spaceId)
          ...
          hs.fnutils.each(
            hs.screen.allScreens(), function(screen)
                local screenWindows = hs.fnutils.filter(
                                        spaceWindows, function(win)
                ...
                if not layoutName or not layouts[layoutName] then
                    ...
                    for index, window in pairs(screenWindows) do
                        local frame = layouts[layoutName](
                                        window, screenWindows, screen, index,
...
module.detectTile = function(win)
    ...
    if module.filters then
        local foundMatch = hs.fnutils.find(
                             module.filters, function(obj)
              local appMatches = ternary(
                                   obj.app ~= nil and app ~= nil, string.match(app, obj.app or ''),
                                   true
                                 )
              local titleMatches = ternary(
                                     obj.title ~= nil and title ~= nil,
                                     string.match(title, obj.title or ''), true
                                   )
...
local saveSettings = function()
    local tilingCache = {}
    local floatingCache = hs.fnutils.map(
                            cache.floating, function(win)
          return win:id()
      end
                          )
