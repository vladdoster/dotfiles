...
obj.updateDisplay = function(cpuUsage)
  ...
  if cpuActive < 100 then cpuGuage = cpuGuage:setStyle({color = obj.emptyBar}, math.ceil(obj.barWidth * cpuActive / 100), #cpuGuage) end
  ...
  batteryGuage = stext.new('Bat ', {font = obj.baseFont, color = {list = 'ansiTerminalColors', name = 'fgBlack'}}) .. batteryGuage
...
obj.show = function(self)
  ...
  if not obj._timer then
    ...
    obj._timer = timer.doEvery(300, function() obj._cpuPoll = host.cpuUsage(obj.timeSlice, obj.updateDisplay) end)
  end
  obj.canvas:show()
