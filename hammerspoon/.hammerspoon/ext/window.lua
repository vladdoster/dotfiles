...
local cache = {
    windowPositions = hs.settings.get(
        'windowPositions') or {}
}
...
module.cycleWindows = function(
    direction, appWindowsOnly,
    screenWindowsOnly)
    ...
    local windows = hs.fnutils.filter(
                        ...
                        function(testWin)
            return testWin ~= nil
                       and testWin:isStandard()
                       and testWin:isVisible()
        end)
...
module.persistPosition =
    function(win, option)
        local windowPositions =
            cache.windowPositions

        -- store position into hs.settings
        if win == 'store' or option
            == 'store' then
            hs.settings.set(
                'windowPositions',
                windowPositions)
        ...
        if option == 'redo' and index
            ~= nil then
            index = math.min(#frames,
                             index + 1)
            win:setFrame(frames[index])
        end
