...
local cache = {windowPositions = hs.settings.get('windowPositions') or {}}
...
module.cycleWindows = function(direction, appWindowsOnly, screenWindowsOnly)
    ...
    if appWindowsOnly and not win then
        ...
        local screenWindows = hs.fnutils.filter(
                                hs.window.visibleWindows(), function(testWin)
...
module.persistPosition = function(win, option)
    ...
    cache.windowPositions[appId] = {index = index, frames = frames}
end
