...
module.forceLaunchOrFocus = function(appName)
    ...
    cache.launchTimer = hs.timer.doAfter(
                          focusTimeout, function()
          local frontmostApp = hs.application.frontmostApplication()
          local frontmostWindows = hs.fnutils.filter(
                                     frontmostApp:allWindows(), function(win)
...
module.smartLaunchOrFocus = function(launchApps)
    ...
    local runningApps = hs.fnutils.map(
                          launchApps, function(launchApp)
    ...
    hs.fnutils.each(
      runningApps, function(runningApp)
          local standardWindows = hs.fnutils.filter(
                                    runningApp:allWindows(), function(win)
                return win:isStandard()
            end
                                  )
