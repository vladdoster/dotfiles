local forceFocus =
    require('ext.window').forceFocus
local highlightWindow = require(
                            'ext.drawing').highlightWindow
local template = require('ext.template')
local log = hs.logger.new('application',
                          'debug')
...
module.activateFrontmost =
    function()
        local frontmostWindow =
            hs.window.frontmostWindow()
        if frontmostWindow then
            frontmostWindow:raise()
                :focus()
        end
    end

-- force application launch or focus
module.forceLaunchOrFocus = function(
    appName)
    local appInstance =
        hs.application.get(appName)
    local isRunning =
        appInstance
            and appInstance:isRunning()
    local focusTimeout = isRunning
                             and 0.05
                             or 1.5
    ...
    cache.launchTimer = hs.timer
            ...
            if frontmostApp:title()
                ~= appName then
                log.d(
                    'Expected app in front: '
                        .. appName
                        .. ' got: '
                        .. frontmostApp:title())
                return
...
module.smartLaunchOrFocus = function(
    launchApps)
    local frontmostWindow = hs.window
                                .frontmostWindow()
    local runningWindows = {}

    launchApps = type(launchApps)
                     == 'table'
                     and launchApps
                     or { launchApps }
    ...
    local runningApps =
        hs.fnutils.map(launchApps,
                       function(
            launchApp)
            return hs.application.get(
                       launchApp)
        end)

    -- create table of sorted windows per application
    hs.fnutils.each(runningApps,
                    function(runningApp)
...
module.allWindowsCount = function(
    appName)
    local _, result =
