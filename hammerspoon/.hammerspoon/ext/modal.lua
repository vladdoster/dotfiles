...
function Modal:new(opts)
    ...
    function obj.modal:entered()
        if obj.exitTimeout > 0 then
            obj.exitTimer = hs.timer.doAfter(
                              obj.exitTimeout, function()
                  obj.modal:exit()
              end
                            )
        end

        hs.notify.new({title = opts.name, subTitle = 'Entered', contentImage = IMAGE_PATH}):send()
    end

    function obj.modal:exited()
        hs.notify.new({title = opts.name, subTitle = 'Exited', contentImage = IMAGE_PATH}):send()
    end
...
function Modal:updateTimeout()
    ...
    if self.exitTimeout > 0 then
        self.exitTimer = hs.timer.doAfter(
                           self.exitTimeout, function()
              self.modal:exit()
          end
                         )
    end
