...
module.isSpaceFullscreenApp = function(spaceID) return spaceID ~= nil and #spaces.spaceOwners(spaceID) > 0 end
...
module.sendToSpace = function(win, spaceModifier)
    ...
    local shouldMoveWindow = hs.fnutils.every({clickPoint ~= nil, not cache.movingWindowToSpace},
    ...
    -- fix for Chrome UI
    if win:application():title() == 'Google Chrome' then clickPoint.y = clickPoint.y - clickPoint.h end
