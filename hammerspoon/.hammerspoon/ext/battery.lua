...
module.batteryNotifications = {
  {
    ...
    fn = function()
      ...
      if not suppressAudio then
        ...
        if volume then audio:setVolume(25) end -- apparently some devices don't have a volume or mute...
  ...
  },
  {
    ...
    fn = function()
      ...
      if not suppressAudio then
        ...
        end):speak('WARNING: BATTERY AT 5%')
      end
...
local updateMenuTitle = function()
  if menuUserData then
    ...
    if amp then
      local text, timeValue = string.format('%+d\n', amp), -999
