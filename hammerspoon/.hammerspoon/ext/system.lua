...
local activateFrontmost = require(
                              'ext.application').activateFrontmost
local bluetooth = require(
                      'hs._asm.undocumented.bluetooth')
local capitalize =
    require('ext.utils').capitalize
local template = require('ext.template')

-- show notification center
-- NOTE: you can do that from Settings > Keyboard > Mission Control
module.toggleNotificationCenter =
    function()
...
module.isDNDEnabled =
    function()
        local _, _, _, rc =
            hs.execute(
                'do-not-disturb status | grep -q "on"',
                true)
        return rc == 0
    end
...
module.toggleDND = function()
    ...
    hs.timer.doAfter(afterTime,
                     function()
        hs.execute('do-not-disturb '
                       .. (isEnabled
                           == true
                           and 'off'
                           or 'on'),
...
module.toggleBluetooth =
    function()
        ...
        local imagePath =
            os.getenv('HOME')
                .. '/.hammerspoon/assets/bluetooth.png'
...
module.toggleWiFi = function()
    ...
    local imagePath =
        os.getenv('HOME')
            .. '/.hammerspoon/assets/airport.png'
...
module.toggleConsole =
    function()
        hs.console.darkMode(
            module.isDarkModeEnabled())
        hs.toggleConsole()
        activateFrontmost()
    end
...
module.displaySleep =
    function()
        hs.task
            .new('/usr/bin/pmset', nil,
                 { 'displaysleepnow' }):start()
    end
