local axuiWindowElement = require(
                              'hs._asm.axuielement').windowElement

local module = {}
local log = hs.logger.new('overrides',
                          'debug');
...
local isResizable = function(win)
    return
        axuiWindowElement(win):isAttributeSettable(
            'AXSize')
end

module.init = function()
    local gridMargin = (hhtwm
                           and hhtwm.margin)
                           or 12
    ...
    hs.grid
        .setGrid('24x15', '1680x1050') -- cell: 70 x 70
    hs.grid
        .setGrid('32x18', '1920x1080') -- cell: 60 x 60
    hs.grid
        .setGrid('32x18', '2560x1440') -- cell: 80 x 80
    hs.grid
        .setGrid('18x32', '1440x2560') -- cell: 80 x 80

    hs.grid
        .setGrid('48x20', '3840x1600') -- cell: 60 x 60
    ...
    hs.grid.set = function(win, cell,
        ...
        if hhtwm then
            local screenMarginFromTiling =
                hhtwm.screenMargin.top
                    - hhtwm.margin / 2

            screenRect.y =
                screenRect.y
                    + screenMarginFromTiling
            screenRect.h =
                screenRect.h
                    - screenMarginFromTiling
        end
        ...
        local cellW =
            screenRect.w / screenGrid.w
        local cellH =
            screenRect.h / screenGrid.h
        ...
        if not isWinResizable then
            ...
            local frameWidth = widthDiv
                                   * cellW
            frameMarginX =
                (frameWidth - winFrame.w)
                    / 2 - margins.w / 2
            frame.w = winFrame.w
        end
        ...
        if cell.x ~= 0 and frame.x
            - screenRect.x + frame.w
            > screenRect.w - maxMargin then
            frame.x =
                screenRect.x
                    + screenRect.w
                    - margins.w - frame.w
        ...
        if cell.y ~= 0
            and (frame.y - screenRect.y
                + frame.h > screenRect.h
                - maxMargin) then
            frame.y =
                screenRect.y
                    + screenRect.h
