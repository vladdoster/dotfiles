...
--- you prefer). Isolate mode is always available and can e toggled manually via `hs.window.highlight.toggleIsolate()`
...
local uiGlobal = {
  frameColor = {1, 0.1, 0.1, 1},
  frameColorInvert = {0, 0, 0, 0},
  frameWidth = 10,
  overlay = true, -- disabled
  overlayColor = {0.1, 0.1, 0.1, 0.1},
  overlayColorInvert = {0, 0, 0, 0},
...
local tflash = timer.delayed.new(0.1, function() rflash:hide() end)
...
local focusedsubs = {
  [windowfilter.windowFocused] = function(w)
    focusedWin = w
    return drawFrame()
  end,
...
local function setMode()
  ...
  for _, r in ipairs{rt, rl, rb, rr} do r:setFillColor(invert and (isolate and isolinv or overinv) or (isolate and isol or over)) end
  ...
  if isolate or hasOverlay then
    ...
    wfOverlay:unsubscribe(focusedsubs)
  end
  if hasFlash then
    wfFlash:subscribe(flashsubs)
  else
...
local function setUiPrefs()
  ...
  tflash:setDelay(ui.flashDuration)
...
function highlight.toggleIsolate(v)
  ...
  if v == nil then
    settings.clear(SETTING_ISOLATE_OVERRIDE)
  else
...
function highlight.start(wfis, wfov)
  ...
  running = true
  if wfov == nil then
    wfov = windowfilter.default
  else
    wfov = windowfilter.new(wfov)
  end
  ...
  if wfis ~= nil then
    if windowfilter.iswf(wfis) then
      wfIsolate = wfis
    else
...
function highlight.stop()
  ...
  if wfIsolate then
    if modulewfIsolate then
      modulewfIsolate:delete()
      modulewfIsolate = nil
    else
      wfIsolate:unsubscribe(isolatesubs)
    end
    wfIsolate = nil
  end
  screenWatcher:stop()
