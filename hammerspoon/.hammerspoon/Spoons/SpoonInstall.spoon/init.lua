...
function obj:_storeRepoJSON(repo,
                            callback,
                            status,
                            body, hdrs)
    local success = nil
            ...
            for i, v in ipairs(json) do
                v.download_url =
                    self.repos[repo]
                        ...
                        .. v.name
                        .. '.spoon.zip'
                self.repos[repo].data[v.name] =
                    v
            end
            self.logger.df(
                'Updated JSON data for repository \'%s\'',
                repo)
            success = true
        else
            self.logger.ef(
...
function obj:_build_repo_json_url(repo)
    if self.repos[repo]
        and self.repos[repo].url then
        self.repos[repo].json_url =
            string.gsub(
                self.repos[repo].url,
                '/$', '')
                .. '/raw/master/docs/docs.json'
        self.repos[repo]
            .download_base_url =
            string.gsub(
                self.repos[repo].url,
        ...
        self.logger.ef(
            'Invalid or unknown repository \'%s\'',
            repo)
        return nil
    end
end
...
function obj:asyncUpdateRepo(repo,
                             callback)
    ...
    if self:_build_repo_json_url(repo) then
        local a, b, c =
            hs.http.get(
                self.repos[repo]
                    .json_url)
        return self:_storeRepoJSON(repo,
                                   nil,
                                   a, b,
                                   c)
    else
    ...
    if (status < 100) or (status >= 400) then
        ...
        if tmpdir then
            local outfile =
                string.format('%s/%s',
                              tmpdir,
                              urlparts.lastPathComponent)
            local f =
                assert(io.open(outfile,
                               'w'))
            f:write(body)
            f:close()
...
function obj:asyncInstallSpoonFromZipURL(
    url, callback)
    local urlparts =
        hs.http.urlParts(url)
    local dlfile =
        urlparts.lastPathComponent
    if dlfile and dlfile ~= ''
        and urlparts.pathExtension
        == 'zip' then
        ...
        self.logger.ef(
            'Invalid URL %s, must point to a zip file',
            url)
        return nil
    end
end
...
--- Returns:
---  * `true` if the installation was successful, `nil` otherwise.
function obj:installSpoonFromRepo(name,
                                  repo,
                                  callback)
    if not repo then repo = 'default' end
    if self:_is_valid_spoon(name, repo) then
        return
            self:installSpoonFromZipURL(
                self.repos[repo].data[name]
                    .download_url)
    end
    return nil
...
function obj:andUse(name, arg)
    ...
    if hs.spoons.use(name, arg, true) then
        ...
        if self.repos[repo] then
            if self.repos[repo].data then
                ...
                if self.use_syncinstall then
                    return
                        load_and_config(
                            nil,
                            self:installSpoonFromRepo(
                                name,
                                repo))
                else
                    self:asyncInstallSpoonFromRepo(
                        name, repo,
                        load_and_config)
                end
            else
                local 
                    update_repo_and_continue =
                    function(_, success)
                        if success then
                            obj:andUse(
                                name,
            ...
            obj.logger.ef(
                'Unknown repository \'%s\' for Spoon',
                repo, name)
        end
    end
end
