...
function obj:_storeRepoJSON(repo, callback, status, body, hdrs)
    ...
    if (status < 100) or (status >= 400) then
        self.logger.ef('Error fetching JSON data for repository \'%s\'. Error code %d: %s', repo, status,
...
function obj:_build_repo_json_url(repo)
    if self.repos[repo] and self.repos[repo].url then
        ...
        self.repos[repo].json_url = string.gsub(self.repos[repo].url, '/$', '') .. '/raw/' .. branch ..
                                        '/docs/docs.json'
        self.repos[repo].download_base_url = string.gsub(self.repos[repo].url, '/$', '') .. '/raw/' .. branch ..
                                                 '/Spoons/'
        return true
    else
...
function obj:asyncInstallSpoonFromZipURL(url, callback)
    ...
    if dlfile and dlfile ~= '' and urlparts.pathExtension == 'zip' then
        hs.http
            .asyncGet(url, nil, hs.fnutils.partial(self._installSpoonFromZipURLgetCallback, self, urlparts, callback))
        return true
    else
    ...
    if self.repos[repo] then
        if self.repos[repo].data then
            if self.repos[repo].data[name] then
                ...
                self.logger.ef('Spoon \'%s\' does not exist in repository \'%s\'. Please check and try again.', name,
                               repo)
            end
        else
            self.logger.ef(
